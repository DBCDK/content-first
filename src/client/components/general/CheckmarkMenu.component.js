...
export const MenuItem = (props) => (
  <div className="list-group-item menu-item" onClick={props.onClick}>
    {props.text && <span className="menu-item-text">{props.text}</span>}
    {props.marked && <span className="glyphicon glyphicon-ok"/>}
  </div>
);
  ...
  render() {
    return (
      <div className="checkmark-menu" tabIndex="1" onBlur={() => this.setState({expanded: false})}>
        <div>
          <span className={`checkmark-menu--expand-btn${this.props.marked ? ' marked' : ''}`}>
            <span className="btn btn-default" onClick={this.props.onClick}>
              {this.props.text}
              {this.props.marked && <span className="glyphicon glyphicon-ok"/>}
            </span>
