...
class ProfileCreateTaste extends React.Component {
  ...
  getImagesFromBelt(beltName, profile) {
    return this.props.belts[beltName]
      .filter(({label}) => profile[beltName].includes(label))
      .map(({image}) => image);
  }
      ...
      return (
        <div>
          ...
          <div className="flex-grid flex-grid-8 flex-grid-6-m square">
            {Object.keys(this.props.profiles).map(taste =>
              this.renderTaste(taste)
            )}
          </div>
        </div>
      );
      ...
      <div className="profile-create">
        ...
        >
          <input
            ...
            name="tastyName"
            value={this.state.tastyName}
            placeholder="Opret en smagsprofil"
            onChange={e => this.setState({tastyName: e.currentTarget.value})}
          />
          <input
            className="btn btn-primary"
            type="submit"
            value="Opret smagsprofil"
          />
        </form>
