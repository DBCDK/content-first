...
import {FETCH_INTERACTIONS, FETCH_INTERACTIONS_ERROR, FETCH_INTERACTIONS_SUCCESS} from "./interaction.reducer";
...
export const interactionMiddleware = store => next => action => {
  ...
  switch (action.type) {
    case ON_LOCATION_CHANGE:{
      ...
      store.dispatch({type: INTERACTION, pid: pid, interaction: 'on_location_change'});
      store.dispatch({type: LOG_INTERACTION, pid: pid, interaction: 'on_location_change'});
      return next(action);
    }
    case ON_SHORTLIST_TOGGLE_ELEMENT: {

      store.dispatch({type: INTERACTION, pid: action.element.book.pid, interaction: 'on_shortlist_toggle'});
      store.dispatch({type: LOG_INTERACTION, pid: action.element.book.pid, interaction: 'on_shortlist_toggle'});
      return next(action);
    }
    case ORDER: {

      store.dispatch({type: INTERACTION, pid: action.book.pid, interaction: 'order'});
      store.dispatch({type: LOG_INTERACTION, pid: action.book.pid, interaction: 'order'});
      return next(action);
    }
    case LIST_TOGGLE_ELEMENT: {

      store.dispatch({type: INTERACTION, pid: action.element.book.pid, interaction: 'list_toggle_element'});
      store.dispatch({type: LOG_INTERACTION, pid: action.element.book.pid, interaction: 'list_toggle_element'});
      return next(action);
    }
  ...
  switch (action.type) {
    case LOG_INTERACTION: {
      try {
        ...
        request
          .post('/v1/object/')
          .send({_type: "INTERACTION", error: 'CLIENT_LOG_ERROR', content: String(e)})
          .end();
      }
      break;
    }
