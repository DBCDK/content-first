...
describe('books', () => {
  describe('parsingMetaDataInjection', () => {
    it('should complain about missing or wrong values in JSON', () => {
      return expect(
        books.parsingMetaDataInjection(wrongBook)
        ...
        expect(problems).to.deep.include(
          'field taxonomy_description is required'
        );
      });
    });
    it('should complain about wrong bibliographic-record id', () => {
      ...
      return expect(
        books.parsingMetaDataInjection(wrongId)
      ).to.be.rejected.then(error => {
        expect(error).to.have.property('title');
        expect(error.title).to.match(
          /bibliographicrecordid cannot be converted to an integer/i
        );
      });
    });
    it('should fill in all values from JSON', () => {
      return expect(books.parsingMetaDataInjection(blendstrup)).to.become({
        ...
        cover:
          'https://moreinfo.addi.dk/2.9/more_info_get.php?lokalid=53188931&attachment_type=forside_stor&bibliotek=870970&source_id=870970&key=d2cc02a57d78c7015725'
      });
    });
  });
  describe('transformMetaDataToBook', () => {
    it('should fit the database schema', () => {
      return expect(
        books.parsingMetaDataInjection(blendstrup)
      ).to.not.be.rejected.then(metaData => {
