...
describe('tags', () => {
  describe('parsingTagsInjection', () => {
    ...
    it('should complain about missing or wrong values in JSON', () => {
      return expect(tags.parsingTagsInjection(wrongTags))
        .to.be.rejected
        .then(error => {
          expect(error).to.have.property('title');
          expect(error.title).to.match(/does not adhere to tags-in\.json/i);
    ...
    it('should fill in all values from JSON', () => {
      ...
      return expect(tags.parsingTagsInjection(goodTags))
        .to.become({
